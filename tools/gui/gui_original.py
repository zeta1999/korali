...
questions = [
    "Optimization", "Sampling", "Bayesian Inference",
...
linktoVariables = [
]  # Stores the information coming from Problem and Solver to update the Variable Frame.
...
def tutorial():
  ...
  B2 = ttk.Button(
      tut,
      text='How do I get Days-timed data?',
      command=lambda: popupmsgwarning('Not yet complited'))  # How do I trade???
  B2.pack()
  B3 = ttk.Button(
      tut,
      text='Graph Question/Help',
      command=lambda: popupmsgwarning('Not yet complited'))
  B3.pack()
...
def explainDescription(description):
  ...
  label = ttk.Label(
      tut,
      text=description[0:int(half)] + '\n' + description[int(half)::],
      font=LARGE_FONT)
  label.pack(side='top', fill='x', pady=10)
  tut.maxsize('950', '600')
  ...
  if 'vector' not in line:
    ...
    if 'string' in line or 'korali' in line:  # == 'std::string
      ...
      if options == 'None':
        ...
        res[texto] = tk.Entry(
            self, width=10, font='Arial 12', textvariable=stringVar[texto])
        res[texto].grid(row=r, column=c + 1, pady=2, padx=6, sticky='w')
        self.corrector = self.register(validarstring)
        res[texto].config(
            validate='focus', validatecommand=(self.corrector, '%P')
        )  # %P represents the parameter we want to pass to validate.
      else:
        stringVar[texto] = tk.StringVar()
        ...
        optionMenu = OptionMenu(
            self, stringVar[texto], *listoptions
        )  #,command = lambda _:getValOptionMenu(stringVar[texto]))
        optionMenu.grid(row=r, column=c + 1, pady=2, padx=6)
        res[texto] = stringVar[texto]
        self.buttonLabel = tk.Button(
            self,
            text=fakedescription,
            font='Arial 11',
            width=20,
            activeforeground='darkcyan',
            bg=color,
      ...
      res[texto].config(
          validate='key', validatecommand=(
              self.corrector,
              '%P'))  # %P represents the parameter we want to pass to validate.
      ...
      stringVar[texto].set("Choose ...")  # default value
      om = OptionMenu(
          self, stringVar[texto], "Yes",
          "No")  #,command = lambda _:getValOptionMenu(stringVar[texto]))
...
##            res[texto] = boolean
      ...
      stringVar[texto] = tk.StringVar()
      res[texto] = tk.Entry(
          self, width=10, textvariable=stringVar[texto], font='Arial 12')
      res[texto].grid(row=r, column=c + 1, pady=2, padx=6, sticky='w')
      self.corrector = self.register(validarstring)
      res[texto].config(
...
def printVariables(self, directorio, DB, cont):
  ...
  if on == 0:
    if linktoVariables0:
      linktoVariables.append(
          linktoVariables0)  # Añadimos las variables que vienen del Problem.
    if linktoVariables1:
      linktoVariables.append(
          linktoVariables1)  # Añadimos las variables que vienen del Solver.
    cont = 2
    # Clean the frame:
      ...
      for llave in variables[part]:
        for var in llave.keys():
          if var == 'Type':
            ...
            options = 'None'
            printdata(self, line, texto, description, fakedescription, ro, co,
                      cont, options)
            ro += 1
  '''
        for part in linktoVariables:
            ...
            if 'Type' not in dicc.keys():
              ...
              for key2 in dicc.keys():
                if key2 == 'Type':
                  ...
                  fakedescription = description
                  printdata(self, dicc[key2], texto, description,
                            fakedescription, r, c, cont, options)
                r += 1
  else:
...
def importFile():
  ...
  if parsedFile != '':
    ...
    for directorio in parsedFile.keys(
    ):  # Loop through the keys of the config file.
  ...
  for key in DB.keys():
    if 'Variables Configuration' in DB[key]['herencia'].keys():
      if DB[key]['herencia']['Variables Configuration']:
        variables[DB[key]
                  ['config']] = DB[key]['herencia']['Variables Configuration']
      else:
    ...
    if len(dirInfo['children']) == 0:
      padre.add_command(
          ...
          command=lambda: [
              ...
              printVariables(bottomrightFrame.tab2, directorio, DB, cont)
          ])
    else:
      padre.add_cascade(label=nombre, menu=subMenu)
      configPath = dirInfo['config']
      if configPath != "NULL" and len(
          dirInfo['children']) == 0:  # SI ES UN LEAF:
        dirs = splitPath(configPath)
        subMenu.add_command(
            label=dirs[len(dirs) - 1],
            command=lambda: [
                printConfig(directorio, DB, cont),
                printVariables(bottomrightFrame.tab2, directorio, DB, cont)
            ])
...
def cascade(mainPath, DB, cont):
  ...
  menus.append(
      titulo
  )  # Añadimos el título del boton, para no repetirlo en los desplegables.

  # Empezar los menus recursivamente llamando a la funcion crearMenu:
  for directorio in DB.keys():
    crearMenu(menuPadre, directorio, DB, cont)
  menus.clear(
  )  # Allows creating different experiments by emptying the menus list.
...
def crearFrameVariables():
  ...
  if len(totalTabs2.tabs()) == 0:
    bottomrightFrame.tab2 = tk.Frame(
        bottomrightFrame, height=450, width=300,
        background='floralwhite')  #florawhite
            ...
            for var in llave.keys():
              ...
              if var == 'Type':
                ...
                if len(description) > 45:
                  ...
                  fakedescription = description
                printdata(self, line, texto, description, fakedescription, ro,
                          co, cont)
            ro += 1
...
def crearTab(self, totalTabs):
  ...
  if contador < 7:
    ...
    self.bottomleftFrame = tk.Frame(
        ...
        height=850,
        borderwidth=3,
        relief='groove')  #width = 950, height = 850
    self.bottomleftFrame.grid(
        column=0, row=1, sticky='nsew', rowspan=5, columnspan=2)
    self.bottomleftFrame.grid_propagate(0)

    self.bottomleftFrame1 = tk.Frame(
    ...
    self.expFrame = tk.Frame(
        self.bottomleftFrame, background='white', width=550,
        height=850)  #width = 950, height = 850
    ...
    self.canvas = Canvas(
        self.bottomleftFrame1,
        width=650,
        height=850,
        bg='azure',
        scrollregion=(0, 0, 1500, 1500))  #yscrollcommand = vbar.set
              ...
              for key2 in dicc.keys():
                ...
                if texto in forbidden:  # IFF Problem, Solver or Variables... skip
                  ...
                  if key2 == 'Type':
                    ...
                    try:
                      options = dicc[
                          'Options']  # options = list of diccionaries.
                    except:
                      options = 'None'

                    fakedescription = description
                    printdata(self.expFrame, dicc[key2], texto, description,
                              fakedescription, r, c, cont, options)
                  r += 1
...
class KORALI(tk.Tk):  #Inherited tk.tk
  ...
  def __init__(
      self, *args,
      **kwargs):  # Self is implied, you don't need to pass self, but is a must.
    ...
    )  # Fill the entire space. Expand = If there is white space, you can expand it.
    ...
    self.homemenu = tk.Menu(
        self.menubar, tearoff=0
    )  # Tearoff = if clicking in the dashedline, we can make it its own window.
    self.homemenu.add_command(
        label='Experiments', command=lambda: self.show_frame(KORALI_Page))
    self.homemenu.add_command(
        label='Second Part', command=lambda: self.show_frame(KORALI_Graph))
    self.homemenu.add_separator()
    self.homemenu.add_command(
        label='Save', command=lambda: popupmsgwarning('SAVE'))
    self.homemenu.add_command(
        label='Save as...', command=lambda: createConfig())
    self.homemenu.add_separator()  # Separator baselr.
    self.homemenu.add_command(
        label='Import File...', command=lambda: importFile())
    self.homemenu.add_command(
        label='Export as...', command=lambda: createConfig())
    ...
    self.experimentmenu.add_command(
        label='New Experiment...',
        command=lambda: crearTab(self, self.totalTabs))
    self.experimentmenu.add_command(
        label='New Variable...',
        command=lambda: crearFrameVariables())  #,self.totalTabs2))
    self.experimentmenu.add_separator()  # Separator baselr.
    self.experimentmenu.add_command(
        label='Delete Experiment...', command=lambda: deleteTab(self.totalTabs)
    )  #self.totalTabs.forget(self.tab))#lambda:deletetab(self,self.totalTabs,contador))
    self.experimentmenu.add_command(
        label='Delete Variable...', command=lambda: deleteVariable(self))
    self.experimentmenu.add_separator()  # Separator baselr.
    self.experimentmenu.add_command(
        label='Clear Experiment...', command=lambda: clearFrame())
    self.menubar.add_cascade(
        label='Experiments Configuration', menu=self.experimentmenu)
    ...
    for F in (StartPage, KORALI_Page, KORALI_Graph):  # List of Pages.
      ...
      frame.grid(
          row=0, column=0,
          sticky='nsew')  # Sticky = Strech everything to north,south,e,w..
...
class KORALI_Graph(tk.Frame):  # Page with Graphs.
  ...
  def __init__(self, parent, controller):
    ...
    button2 = tk.Label(
        self,
        text="This is the frame for the next part",
        width=150,
        height=10,
        relief=tk.RAISED)
...
class StartPage(tk.Frame):  # inherit everything from tk.Frame
  ...
  def __init__(self, parent, controller):
    ...
    button1 = tk.Button(
        ...
        font=HUGE_FONT,
        command=lambda: controller.show_frame(KORALI_Page)
    )  # lambda creates a quick thronaway function.
