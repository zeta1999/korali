...
def processTutorialSubFolder(folder):
  ...
  with open(tutorialsDir + '/README.rst', 'r') as file:
    tutorialsString = file.read()
  ...
  for tutorialName in sorted(tutorialNames):
    ...
    if (os.path.isdir(tutorialPath)):
      ...
      with open(tutorialReadmeFile, 'r') as file:
        tutorialReadmeString = file.read()
      tutorialTitle = tutorialReadmeString.partition('\n')[0]
      tutorialDstPath = folder + '/' + tutorialName + '.rst'
      tutorialsString += '    ' + folder + '/' + tutorialName + '\n'
      tutorialOutputString = '.. _' + ''.join(
          i for i in tutorialName.replace('.', '') if not i.isdigit()) + ':\n\n'
      if ('examples' in folder):
        tutorialOutputString += '**Tutorial Source Code:** `https://github.com/cselab/korali-apps/tree/master/' + tutorialName + '/ <https://github.com/cselab/korali-apps/tree/master/' + tutorialName + '/>`_\n\n'
      else:
      ...
      with open('../tutorials/' + tutorialDstPath, 'w') as file:
        tutorialReadmeString = file.write(tutorialOutputString)
