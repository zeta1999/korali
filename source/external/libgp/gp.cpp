...
void GaussianProcess::compute()
{
  ...
  // printf("COMPUTE \n");
  // auto q=covf().get_loghyper();
  // printf("q =  ");
  // for(size_t j=0; j<q.size(); j++) printf("%20.20lf  ",q[j] );printf("\n");
...
double GaussianProcess::log_likelihood()
{
  ...
  // auto q=covf().get_loghyper();
  // printf("L >>> ");
  // for(size_t j=0; j<q.size(); j++) printf("%lf  ",q[j] );printf("\n");

  double res = -0.5 * y.dot(alpha) - 0.5 * det - 0.5 * n * log2pi;
