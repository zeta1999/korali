...
{
 double ccLogPrior = _k->_problem->evaluateLogPrior(&_chainCandidateParameters[c*_k->N]);
 ...
 if ( L >= 1.0 || L > gsl_ran_flat(range, 0.0, 1.0) ) {
   for (size_t i = 0; i < _k->N; i++) _chainLeaderParameters[c*_k->N + i] = _chainCandidateParameters[c*_k->N + i];
 ...
 _currentChainStep[c]++;
 if (_currentChainStep[c] > _defaultBurnIn ) updateDatabase(&_chainLeaderParameters[c*_k->N], _chainLeadersLogLikelihoods[c]);
