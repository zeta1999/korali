...
void CMAES::initialize()
{
 ...
 if (_constraintsDefined)
 {
  if( (_globalSuccessLearningRate <= 0.0) || (_globalSuccessLearningRate > 1.0) ) 
    koraliError("CMAES Error: Invalid Global Success Learning Rate (%f), must be greater than 0.0 and less than 1.0\n",  _globalSuccessLearningRate );
  if( (_targetSuccessRate <= 0.0) || (_targetSuccessRate > 1.0) )
    koraliError("CMAES Error: Invalid Target Success Rate (%f), must be greater than 0.0 and less than 1.0\n",  _targetSuccessRate );
  if(_covarianceMatrixAdaptionStrength <= 0.0) 
    koraliError(" CMAES Error: Invalid Adaption Size (%f), must be greater than 0.0\n", _covarianceMatrixAdaptionStrength );
...
void CMAES::checkMeanAndSetRegime()
{
  ...
  Korali::Model data;
  for (size_t i = 0; i < _k->N; i++) data._computationalVariables.push_back(_currentMean[i]);
...
void CMAES::sampleSingle(size_t sampleIdx)
{
  for (size_t d = 0; d < _k->N; ++d)
  {
   ...
   if (_isDiagonal) {
     ...
     _samplePopulation[sampleIdx * _k->N + d] = _currentMean[d] + _sigma * _bDZMatrix[sampleIdx*_k->N+d];
   }
...
void CMAES::updateSigma()
{
 ...
 double _upperBound = sqrt(_trace/_k->N);
 ...
 if(_sigma > _upperBound)
 {
  ...
  if( _isSigmaBounded ) _sigma = _upperBound;
 }
...
bool CMAES::checkTermination()
{
 ...
 size_t iAchse = 0;
 size_t iKoo = 0;
 /* Component of _currentMean is not changed anymore */
 ...
 if (!_isDiagonal )
 {
    ...
    for (iKoo = 0; iKoo < _k->N; ++iKoo){
      if (_currentMean[iKoo] != _currentMean[iKoo] + fac * _covarianceEigenvectorMatrix[iKoo*_k->N+iAchse])
      break;
    }
    if (iKoo == _k->N)
    {
 ...
 for (iKoo = 0; iKoo < _k->N; ++iKoo)
 {
  if (_currentMean[iKoo] == _currentMean[iKoo] + _minStandardDeviationStepFactor*_sigma*sqrt(_covarianceMatrix[iKoo*_k->N+iKoo]) )
  {
   isFinished = true;
...
{
 koraliLog(KORALI_MINIMAL, "CMAES Generation %zu\n", _k->currentGeneration);
 ...
 koraliLog(KORALI_NORMAL, "Covariance Eigenvalues: Min = %+6.3e -  Max = %+6.3e\n", _minimumCovarianceEigenvalue, _maximumCovarianceEigenvalue);
  ...
  for (size_t d = 0; d < _k->N; d++) koraliLogData(KORALI_DETAILED, "         %s = (%+6.3e, %+6.3e)\n", _k->_variables[d]->_name.c_str(), _currentMean[d], _bestEverCoordinates[d]);
