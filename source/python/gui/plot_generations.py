...
if (seaborn_avail): sns.set_style("darkgrid")

plt.figure('CMA-ES live diagnostics: {0}'.format(resultdir),figsize=(8,8))
...
while( os.path.isfile( 'korali00000/s{1}.json'.format(resultdir, str(idx).zfill(5))) ):
    ...
    with open(path) as f:
        ...
        fval.append(state['CurrentBest']) # or maybe BestEver
        ...
        for i in range(numdim):
            ...
            Cdiag[i].append(state['CovarianceMatrix'][i][i])  # Evals
    ...
    plt.yscale('log')
    for i in range(numdim):
        ax223.plot(numeval, Cdiag[i], color = colors[i])
