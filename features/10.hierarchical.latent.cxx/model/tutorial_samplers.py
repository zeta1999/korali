...
class Example925Sampler:
    ...
    def __init__(self, sigma, omega, N, data):
        ...
        assert type(self.data) == list or (self.numberLatent == len(self.data.flatten()) )
...
#         self.S_func = S_
#         self.phi_func = phi_
#         assert (len(initialLatentValues) == numberLatentVars ) ,"number of latent variables should match the dimension of their initial values"
#         assert (len(initialHyperparams) == numberHyperparams ) ,"number of hyperparameters should match the dimension of their initial values"
#
