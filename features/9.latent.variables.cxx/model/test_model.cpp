...
int test_distribution_1()
{
  ...
  std::function<void(korali::Sample & s)> distrib1_S = [&distrib1](korali::Sample &s) -> void { distrib1.S(s); };
  std::function<void(korali::Sample & s)> distrib1_zeta = [&distrib1](korali::Sample &s) -> void { distrib1.zeta(s); };
  std::function<void(korali::Sample & s)> distrib1_phi = [&distrib1](korali::Sample &s) -> void { distrib1.phi(s); };
  ...
  mus.push_back(mu0);
  points.push_back({{0, 0}, {0.5, 0.25}, {-1, 0.01}});
  hyperparams.push_back({0.5});
  mus.push_back(mu2);
  points.push_back({{0, 9}, {0.9, 10.5}});
  hyperparams.push_back({4.});
  mus.push_back(mu1);
  points.push_back({{0, 9}, {0.9, 10.5}, {4.0, 4.01}, {3., 2.5}, {-0.5, 1.9}, {-0.5, 1.9}});
  ...
  for (int i = 0; i < hyperparams.size(); i++)
  {
    ...
    distrib1_S = [&distrib1](korali::Sample &s) -> void { distrib1.S(s); };
    distrib1_zeta = [&distrib1](korali::Sample &s) -> void { distrib1.zeta(s); };
    distrib1_phi = [&distrib1](korali::Sample &s) -> void { distrib1.phi(s); };
    ...
    p_from_model_direct = std::exp(-_zetaValue + std::inner_product(std::begin(_sValues), std::end(_sValues), std::begin(_phiValues), 0.0));
...
int test_distribution_2()
{
  ...
  std::function<void(korali::Sample & s)> distrib2_S = [&distrib2](korali::Sample &s) -> void { distrib2.S(s); };
  std::function<void(korali::Sample & s)> distrib2_zeta = [&distrib2](korali::Sample &s) -> void { distrib2.zeta(s); };
  std::function<void(korali::Sample & s)> distrib2_phi = [&distrib2](korali::Sample &s) -> void { distrib2.phi(s); };
  ...
  for (int i = 0; i < assignments.size(); i++)
  {
    ...
    /* Update S, zeta, phi to the new set of points (needed?)*/
    distrib2_S = [&distrib2](korali::Sample &s) -> void { distrib2.S(s); };
    distrib2_zeta = [&distrib2](korali::Sample &s) -> void { distrib2.zeta(s); };
    distrib2_phi = [&distrib2](korali::Sample &s) -> void { distrib2.phi(s); };
