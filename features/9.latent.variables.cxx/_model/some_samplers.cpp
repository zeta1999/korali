...
MCMCLatentSampler::MCMCLatentSampler(int numberLatentVars, int numberHyperparams, std::vector<double> initialLatentValues, std::vector<double> initialHyperparams, std::function<void(korali::Sample &)> zeta_, std::function<void(korali::Sample &)> S_, std::function<void(korali::Sample &)> phi_, bool sample_discrete_, int min_if_discrete_, int max_if_discrete_)
{
  zeta_func = zeta_;
  S_func = S_;
  phi_func = phi_;
...
void MCMCLatentSampler::sampleLatent(korali::Sample &kSample)
{
  ...
  {
    fprintf(stderr, "[Error] Implementation error, number of latent variables at initialization does not fit to what was passed as variable");
    exit(-1);
  }
  ...
  e["Problem"]["Probability Function"] = [hparams = hyperparameters, this](korali::Sample &s) -> void {
    ...
    {
      fprintf(stderr, "[Error] Implementation error, latent variable vector had wrong size");
      exit(-1);
    }

    if (hparams.size() != numberHyperparameters)
    {
      fprintf(stderr, "[Error] Implementation error, hyperparameter vector had wrong size");
      exit(-1);
    }
    ...
    if (!s.contains("S"))
    {
      fprintf(stderr, "[Error] The specified likelihood model did not assign the value: 'S' to the sample.\n");
      exit(-1);
    } // @suppress("Invalid arguments")
    if (!s.contains("zeta"))
    {
      fprintf(stderr, "[Error] The specified likelihood model did not assign the value: 'zeta' to the sample.\n");
      exit(-1);
    }
    if (!s.contains("phi"))
    {
      fprintf(stderr, "[Error] The specified likelihood model did not assign the value: 'phi' to the sample.\n");
      exit(-1);
    }
    ...
    double logP_of_x = -_zetaValue + std::inner_product(std::begin(_sValues), std::end(_sValues), std::begin(_phiValues), 0.0);
    s["P(x)"] = logP_of_x;
  };
