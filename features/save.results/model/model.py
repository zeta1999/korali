...
#by Korali.
  samplePHs = {} sampleTemps = {} bananaCounts = {} orangeCounts = {} appleCounts = {} harvestQualities = {}
...
#Gathering sample variable values
  ph = d["Parameters"][0] temp = d["Parameters"][1] sampleId = d["Sample Id"]
...
#Storing QOIS
                                                                                        samplePHs[sampleId] = ph sampleTemps[sampleId] = temp bananaCounts[sampleId] = b orangeCounts[sampleId] = o appleCounts[sampleId] = a harvestQualities[sampleId] = x

#Store QoI
                                                                                      d["Apples"] = a d["Bananas"] = b

#Returning evaluation to Korali
                                                                                      d["P(x)"] = x

                                                                                      def getHarvestQuality(b, o, a) : return b * 1.2 + o * 0.9 + a * 1.1

                                                                                                            def getBananaCount(ph, temp) : a = 100 b =((ph - 7.3)) * * 2 +((temp - 35) * 0.1) * * 2 c = 2 t = a * math.exp(- b / c) return math.floor(t)

                                                                                                                                                                                                                             def getOrangeCount(ph, temp) : a = 50 b =((ph - 6.6)) * * 2 +((temp - 27) * 0.1) * * 2 c = 2 t = a * math.exp(- b / c) return math.floor(t)

                                                                                                                                                                                                                                                                                                                                             def getAppleCount(ph, temp) : a = 90 b =((ph - 8.6)) * * 2 +((temp - 20) * 0.1) * * 2 c = 2 t = a * math.exp(- b / c) return math.floor(t)
