...
cp run-mcmc.py  __test-mcmc.py; check_result 
...
if [[ $testMode == "fast" ]]; then
 ...
 sed -e 's%\["Max Samples"\]%\["Max Samples"\] = 5 #%g' run-mcmc.py  > __test-mcmc.py; check_result

fi

##### Running Tests

python3 ./__test-mcmc.py; check_result
