...
constants = {
    ...
    "Cdt":
        0.25,  # time step constraint coefficient: sonic and acceleration terms
...
def simulation(u, plasma_par, hemogl_par, rbc_par, mesh_par, sim_par, ply_dir):
  ...
  plasma_pv = mir.ParticleVectors.ParticleVector(
      'outer', mass=plasma_par['mass'])
  ...
  rbc_pv = mir.ParticleVectors.MembraneVector(
      'rbc', mass=mesh_par['mass'], mesh=mesh_rbc)
  ic_rbc = mir.InitialConditions.Membrane([[
      sim_par['domain'][0] * 0.5, sim_par['domain'][1] * 0.5,
      sim_par['domain'][2] * 0.5, 1.0, 0.0, 0.0, 0.0
  ]])
  ...
  hemogl_pv = u.applyObjectBelongingChecker(
      checker=checker,
      pv=plasma_pv,
      inside='inner',
      correct_every=sim_par['correctfreq'])
  ...
  # ~~~ Interactions
  dpd_int = mir.Interactions.Pairwise(
      'dpd',
      rc=plasma_par['rc'],
      kind='DPD',
      a=plasma_par['alpha'],
      gamma=plasma_par['gamma'],
      kBT=plasma_par['kbt'],
      power=plasma_par['kpow'])
  rbc_int = mir.Interactions.MembraneForces(
      "int_rbc", **rbc_par, stress_free=True)
  ...
  u.registerPlugins(
      mir.Plugins.createDumpMesh(
          'rbcs', rbc_pv, dump_every=sim_par['dumpfreq'], path=ply_dir))
...
def run_korali(comm_address,
  ...
  run(comm_address, gammaC, NDATA, TEND, kT_s, T_p, etao_p, etai_p, mu_p, Ka_p,
      kb_p, area_p, volume_p, verbose, dryrun, ini_mesh_fname, ref_mesh_fname,
      ply_dir)
...
def run(comm_address,
  ...
  dt_m_visc = D.get_dt_viscous(
      hm, max([etao_s, etai_s, etam_s / hm]), rho_s, C=Cdt_visc)
  ...
  rbc = R.set_rbc_params_ade_lwm(Gka_s, Gkv_s, A_s, V_s, gT, gC, kT_rbc, Ka_s,
                                 a3, a4, mu_s, b1, b2, kb_s, C0_s, kade_s,
                                 DA0D_s)
