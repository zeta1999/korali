...
parser.add_argument(
    '--dataFolder', '-df', default='data', help='Load results from this folder')
...
ax[0].plot(
    d['x-data'], d['y-data'], 'o', lw=2, label='infected (data)', color='black')
...
for k, y in enumerate(var):
  ax[k].plot(d['x-axis'], d[y]['Mean'], '-', lw=2, label='Mean', color='blue')
  ax[k].plot(
      d['x-axis'], d[y]['Median'], '-', lw=2, label='Median', color='black')
  ...
  for x in d[y]['Intervals']:
    ...
    p = 100 * x['Percentage']
    ax[k].fill_between(
        d['x-axis'], p1, p2, alpha=0.5, label=f' {p:.0f}% credible interval')
