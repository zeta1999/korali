...
def intervals_cumsum(iv_daily, exp=2.):
  ...
  for local_iv_daily in iv_daily['Intervals']:
    ...
    local_iv['High Interval'] = cumsum_high(local_iv_daily['High Interval'],
                                            iv_daily['Mean'], iv['Mean'])
...
def compute_intervals(args, jskPropagation, jsData):
  ...
  for k in range(Nv):
    varNames.append(
        jskPropagation['Samples'][0]['Saved Results']['Variables'][k]['Name'])
    derived.append(jskPropagation['Samples'][0]['Saved Results']['Variables'][k]
                   ['derived'])
  ...
  for i, x in enumerate(varNames):
    vars[x] = []
    for k in range(Ns):
      vars[x].append(jskPropagation['Samples'][k]['Saved Results']['Variables']
                     [i]['Values'])
  ...
  if sir.likelihood == sir.Likelihood.nbin:
    ...
    raise NotImplementedError()
  for x in varNames:
    vars_ci[x] = compute_credible_intervals(
        vars[x], std, sir.likelihood, r, ns=Nsp)

  for x, d in zip(varNames, derived):
    if d:
      if d[0] == 'cumsum':
        vars_ci[x] = compute_credible_intervals(
            vars[d[1]], std, sir.likelihood, r, ns=Nsp, cumsum=True)
      elif d[0] == 'diff':
