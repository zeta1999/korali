...
def generate_normal_data(N=None, x=None):
  ...
  all_data_ids = np.array([np.arange(N) for _ in range(Nx)
                          ]).flatten()[:, np.newaxis]
  ...
  with open('all_data.txt', 'w') as fd:
    ...
    fd.writelines([
        '%f \t %f \t %f\n' % (id, x_, y_)
        for id, x_, y_ in zip(all_data_ids, all_data_x, all_data_y)
    ])

  with open("all_data_info.txt", "w") as fd:
    fd.write(
        f'N: {N}\t Mean: [{mean_1}, {fixed_val_2}] (2nd is the noise sdev in the model)\n'
    )
